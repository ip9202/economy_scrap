...
- **이벤트 스터디 분석**: 금리 변동 전후 ±14일 뉴스 논조 변화 추적 (역사적 전체 기간 표시)
- **한/미 금리 비교**: ECOS 한국 기준금리 + FRED 미국 Federal Funds Rate 동시 시각화
- **최근 금리 결정 카드**: 최신 금리 결정과 직전 결정을 인상/인하/동결 표시 및 %p 비교
- **다음 발표일 배너**: 한국은행(BOK) 및 FOMC 다음 금리 결정 예정일 실시간 표시
- **대시보드 시각화**: 실시간 뉴스 스탠스 시계열, 기준금리(한/미), 이벤트 분석 결과 표시
...
- **rate_series.csv**: Bank of Korea base rate time series (한국 기준금리)
- **us_rate_series.csv**: US Federal Funds Rate time series from FRED (월별)
...
- `GET /api/data/news-daily`: 일별 뉴스 스탠스 집계 (날짜 범위 필터)
- `GET /api/data/rate-series`: 한국 기준금리 시계열 (날짜 범위 필터)
- `GET /api/data/us-rate-series`: 미국 Fed Funds Rate 시계열 (FRED, 월별)
- `GET /api/data/events`: 감지된 금리 변동 이벤트 (날짜 범위 필터)
- `GET /api/data/event-study`: 이벤트 스터디 분석 결과 (전체 역사 데이터)
- `GET /api/data/statistics`: 통계 요약 (총 기사수, 평균 감성, 최근 금리 결정 상세)
- `GET /api/data/news-articles`: 개별 뉴스 기사 (페이지네이션)
- `GET /api/data/next-meetings`: 한/미 다음 금리 결정 예정일
- `POST /api/data/refresh`: 파이프라인 실행 (비동기 Job)
- `GET /api/data/refresh/status/{job_id}`: 비동기 Job 상태 조회
